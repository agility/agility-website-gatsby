...
const LogoListingModule = ({ item }) => {
  ...
  const classSection = `module LogoListingModule animation  ${
    item.customFields.darkMode && item.customFields.darkMode === "true"
      ? "dark-mode bg-17 text-white"
      : ""
  }`
    ...
    return (
      <div className={className} key={idx}>
        <div className="d-block">
          <AgilityImage image={key.customFields.logo} loading="lazy" />
  ...
  const initLogo = () => {
    ...
    inter = setInterval(() => {
      if (
        section.classList.contains("set-animation") &&
        section.querySelectorAll(".slick-list").length
      ) {
        clearInterval(inter)
        ...
        <div className="container">
          ...
          {logos.length > 0 && (
            <div className="slider-lazy list-logos-slide anima-bottom">
              <Slider {...settings}>{listLogos}</Slider>
            </div>
          )}
        </div>
      </section>
