...
const BestofBothWorldsModule = ({ item }) => {
  ...
  const classSection = `module mod-HIW BestofBothWorldsModule ${
    fields.darkMode && fields.darkMode === "true" ? "dark-mode" : ""
  }`
  ...
  const initComparisons = () => {
    ...
    // let inter
    thisModuleRef.current
      .querySelectorAll(".best-HIW")[0]
  ...
  const displayWindowSize = () => {
    if (imgOverlayRef.current.length) {
      ...
      compSliderRef.current.style.left =
        ((img.offsetWidth - 30.5 - comparImg) / wCurent) * 100 + "%"
    }
  }
  ...
  const compareImages = img => {
    ...
    window.addEventListener("mouseup", slideFinishRef, { passive: true })
    compSliderRef.current.addEventListener("touchstart", slideReady, {
      passive: true,
    })
    window.addEventListener("touchend", slideFinishRef, { passive: true })
    function slideReady(e) {
      e.preventDefault()
      clicked = 1
    ...
    function slideFinish() {
      ...
      let comparImg = (1 / Math.tan((69 * Math.PI) / 180)) * h
      slide((wrap.offsetWidth + comparImg * 2 - 8) / 2)
      window.removeEventListener("mousemove", slideMove, { passive: true })
      window.removeEventListener("touchmove", slideMove, { passive: true })
      setTimeout(() => {
        wrap.classList.remove("add-transition")
      }, 350)
    }

    document.getElementsByClassName("healine-v1")[0].addEventListener(
      "click",
      () => {
        const comparImg =
          (1 / Math.tan((69 * Math.PI) / 180)) * wrap.offsetHeight
        wrap.classList.add("add-transition")
    ...
    document.getElementsByClassName("healine-v2")[0].addEventListener(
      "click",
      () => {
        const comparImg =
          (1 / Math.tan((69 * Math.PI) / 180)) * wrap.offsetHeight
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("resize", displayWindowSize, { passive: true })
      // compSliderRef.current.removeEventListener('mousedown', slideReadyRef.current);
      window.removeEventListener("mouseup", slideFinishRef, { passive: true })
      // compSliderRef.current.removeEventListener('touchstart', slideReadyRef.current);
      window.removeEventListener("touchend", slideFinishRef, { passive: true })
    }
  }, [])
  ...
  useEffect(() => {
    ...
    animationElementInnerComponent(thisModuleRef.current)
    window.addEventListener("scroll", scrollEventFunc, { passive: true })

    return () => {
      window.removeEventListener("scroll", scrollEventFunc, { passive: true })
    }
  }, [])
    ...
    return (
      <div className={className} key={key.contentID}>
        <input
          ...
          defaultChecked={true}
        ></input>
        <label htmlFor={idInput}>{key.customFields.title}</label>
        <br></br>
      </div>
    )
  })
    ...
    return (
      <div className={className} key={key2.contentID}>
        <input
          ...
          defaultChecked={true}
        ></input>
        <label htmlFor={idInput}>{key2.customFields.title}</label>
        <br></br>
      </div>
    )
  })
    ...
    <React.Fragment>
      ...
      <section className={classSection} ref={thisModuleRef}>
        <div className="container animation">
          ...
          <div className="wrap-video-HIW ps-rv anima-bottom delay-2">
            <div className={`wrap-video ps-rv ${isPaused ? "is-show" : ""}`}>
              {isPaused && <Video />}
              <div
                className=" text-decoration-none has-popup ps-as bg bg-bottom-center"
                allow="autoplay"
                src={urlThumbnail}
        ...
        <div className="best-HIW animation anima-bottom">
            ...
            <div className="middle-HIW last-mb-none text-center d-md-flex align-items-center">
              <div className="lazy compare-2img">
                <LazyLoad offset={Helpers.lazyOffset}>
                  <img
                    src="/images/bg-bottom2.svg"
                    className="bg-2img-top"
                    alt="image"
                    loading="lazy"
                  ></img>
                </LazyLoad>
                ...
                <div className="img-comp-container">
                  <div className="img-comp-img">
                    {rightGroupName && (
                      <React.Fragment>
                        <img
                          src="/images/image-fake.jpg"
                          className="img-primary"
                          alt={rightGroupName.text}
                          onLoad={() => {
                            compareImages(imgOverlayRef.current)
                          }}
                        ></img>
                        <LazyLoad offset={Helpers.lazyOffsetRes}>
                          <img
                            src="/images/image-author-2.png"
                            className="img-second-author"
                            alt={rightGroupName.text}
                            loading="lazy"
                          ></img>
                        </LazyLoad>
                  ...
                  <div className="img-comp-slider" ref={compSliderRef}>
                    ...
                    >
                      ...
                      <path
                        id="Path"
                        d="M18.548,23.025H7.466L15.819,8.6l8.353,14.428,2.436,4.3h5.03L15.818,0,0,27.324H20.489Z"
                        transform="translate(14.338 14.338)"
                        fill="#ffc414"
                      />
                    </svg>
                  </div>
                  ...
                  >
                    {leftGroupName && (
                      <React.Fragment>
                        ...
                        <LazyLoad offset={Helpers.lazyOffsetRes}>
                          <img
                            src="/images/image-dev-2.png"
                            className="img-second-dev"
                            alt={leftGroupName.text}
                          ></img>
                        </LazyLoad>
                      </React.Fragment>
          ...
          {(leftGroupedFeatures.length > 0 ||
            rightGroupedFeatures.length > 0) && (
