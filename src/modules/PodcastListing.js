  ...
  <StaticQuery
    query={graphql`
      query PodcastListing {
        allAgilityPodcast(
          filter: { properties: { referenceName: { eq: "podcast" } } }
          sort: { order: DESC, fields: customFields___date }
        ) {
          nodes {
            contentID
            languageCode
    ...
    render={queryData => {
      ...
      return <PodcastListing {...viewModel} />
    }}
  />
)
...
const PodcastListing = ({ items }) => {
    ...
    return (
      <div className="podcast-episode" key={"podcastlst-" + preItem.contentID}>
        <Link to={item.url}>
          {item.image && (
            <div className="image">
              ...
              <img
                src="https://static.agilitycms.com/layout/img/podcast.svg"
                alt=""
                className="podcast"
                loading="lazy"
              />
        ...
        <div className="content">
          ...
          <div
            className="text"
            dangerouslySetInnerHTML={renderHTML(item.excerpt)}
          ></div>
  ...
  return <div className="podcast-listing">{podcasts}</div>
}
