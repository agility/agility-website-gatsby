...
export default props => (
  <StaticQuery
    ...
    render={queryData => {
      ...
      return <NewGlobalHeader {...viewModel} />
    }}
  />
)
...
class NewGlobalHeader extends Component {
  ...
  componentDidMount() {
    ...
    menus.map(item => {
      if (
        item.customFields?.uRL?.href === pathname ||
        pathname.indexOf(item.customFields?.uRL?.href) !== -1
      ) {
    ...
    menus.map(item => {
      item.subNavigation.map(subItem => {
        if (
          subItem.customFields?.uRL?.href === pathname ||
          pathname.indexOf(subItem.customFields?.uRL?.href) !== -1
        ) {
    ...
    const listNavLv1 = this.header.querySelectorAll(
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.openMenu !== this.state.openMenu ||
      prevState.menuLv2Opening !== this.state.menuLv2Opening
    ) {
      setTimeout(() => {
        forceCheck()
      }, 300)
  ...
  clickAwebinar() {
    document.addEventListener("click", event => {
      ...
      if (
        target.classList.contains("link-line") &&
        target.offsetParent.classList.contains("box-message")
      ) {
  ...
  hiddenMessage() {
    this.mainNode.style.paddingTop =
      this.heightHeaderBackup -
      (this.boxMessage ? this.boxMessage.offsetHeight : 0) +
      "px"
